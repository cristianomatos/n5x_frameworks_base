...
import com.android.internal.util.cm.Converter;
...
public class PieController implements BaseStatusBar.NavigationBarCallback, PieView.OnExitListener,
    ...
    private void getCustomActionsAndConstruct(ContentResolver resolver,
        ...
        if (!secondLayer) {
            mMenuButton = constructItem(1, NavigationRingConstants.ACTION_MENU,
                    NavigationRingConstants.ACTION_NULL,
    ...
    private void addItemToLayer(PieSliceContainer layer, ButtonConfig buttonConfig,
        ...
        if (buttonConfig.getClickAction().equals(NavigationRingConstants.ACTION_HOME)) {
            layer.addItem(constructItem(buttonWidth,
                    NavigationRingConstants.ACTION_KEYGUARD_SEARCH,
                    buttonConfig.getLongpressAction(),
                    NavigationRingConstants.ICON_EMPTY,
                    minimumImageSize));
        }
    ...
    private PieItem constructItem(int width, String clickAction, String longPressAction,
        ...
        if (!longPressAction.equals(NavigationRingConstants.ACTION_NULL)) {
    ...
    private int setPieItemIcon(ImageView view, String iconUri, String clickAction) {
        ...
        if (iconUri != null && !iconUri.equals(NavigationRingConstants.ICON_EMPTY)
            && !iconUri.startsWith(NavigationRingConstants.SYSTEM_ICON_IDENTIFIER)) {
            if (clickAction.equals(NavigationRingConstants.ACTION_BACK)) {
    ...
    protected void setNavigationIconHints(int hints, boolean force) {
        ...
        for (int j = 0; j < 2; j++) {
            item = findItem(NavigationRingConstants.ACTION_BACK, j);
            if (item != null) {
    ...
    protected void setDisabledFlags(int disabledFlags, boolean force) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            item = findItem(NavigationRingConstants.ACTION_HOME, j);
            if (item != null) {
                item.show(!disableHome);
                // If the homebutton exists we can assume that the keyguard
                // search button exists as well.
                item = findItem(NavigationRingConstants.ACTION_KEYGUARD_SEARCH, j);
                item.show(disableHome);
            }
            item = findItem(NavigationRingConstants.ACTION_RECENTS, j);
            if (item != null) {
    ...
    public void onLongClick(PieItem item) {
        ...
        ActionTarget ac = new ActionTarget(mContext);
        if (!ac.isActionKeyEvent(type)) {
    ...
    public void onClick(PieItem item) {
        ...
        if (!type.equals(NavigationRingConstants.ACTION_MENU)) {
